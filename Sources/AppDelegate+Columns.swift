...
extension AppDelegate {
    ...
    func rebuild(rows: [AgentRow]?) {
        ...
        // Minimized keeps the status row (the blocked/working summary is what makes the strip
        // useful); in the full layout it lives inside the sessions section, under its bar.
        if minimized, let status = status { views.append(status) }
