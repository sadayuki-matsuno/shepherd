...
- **Rich cards** — AI-generated task title, permission-mode / model / advisor (`+OPUS` when the session runs with `--advisor`) / runtime chips (what the session runs on: zellij, VS Code, Ghostty, a headless `claude -p` …), context-window gauge, elapsed time, branch, changed-file count, issue number (from the branch), PR number + CI state (via `gh`), links to Artifacts the session published, and a one-line "what Claude is doing right now"
