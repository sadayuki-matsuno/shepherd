...
- **Rich cards** — AI-generated task title, permission-mode / model / advisor (`+OPUS` when the session runs with `--advisor`) / runtime chips (what the session runs on: zellij, VS Code, Ghostty, a headless `claude -p` …), context-window pie (yellow ≥ 60%, red ≥ 85%), elapsed time, changed-file count, issue number (from the branch), PR number + CI state (via `gh`), and links to Artifacts the session published
