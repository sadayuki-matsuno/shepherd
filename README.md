...
- **Family trees, not a flat list** — teammates, subagents and child sessions nest under their parent card while they work; idle teammates (done, but still on the team roster and revivable) are counted on the family strip and listed on hover; finished background records fold into an archive lane
...
- `~/.claude/teams/<team>/` — the agent-team roster (who is still a member, i.e. an idle teammate that a message can revive) and the per-member inbox files, which are also how the teammate actions deliver: Shepherd appends a message in the exact shape the harness itself writes, and the harness picks it up.
