...
- **アプリの性質**: `KeyablePanel`（NSPanel サブクラス・borderless / nonactivatingPanel）＋ LSUIElement。**このアプリは決してアクティブにならない**。これが地雷の温床（後述）。borderless ウィンドウは素では key になれず**パネル上のテキスト入力が全滅する**ため `canBecomeKey=true` を明示（インライン検索の実装で発覚）— `becomesKeyOnlyIfNeeded=true` が効いているのでテキスト欄クリック時だけ key になり、カードクリックでフォーカスを奪うことはない。
...
- **盤面2セクション + Artifact シェルフ**（`Sources/AppDelegate+Shelf.swift` / `Sources/ArtifactIndex.swift`・2026-07-17 採用）: 盤面は「⌄ セッション — N」「⌄ ARTIFACTS — N」の折りたたみバー2本に分割（`sessionsSectionBar` / `artifactsSectionViews`・状態は defaults `sessionsCollapsed` / `artifactsBarCollapsed`）。セッションバーは畳むと状態サマリ（応答待ち/作業中ピル＋A9クォータ警告チップ）がバー上に移る。ARTIFACTS 枠がシェルフの唯一の面（初版のヘッダー棚ボタン＋ポップオーバーはユーザー判断で撤去）: 検索フィールド＋repo 簡易フィルター（ポップアップ: すべて/各repo/帰属不明）＋フラット一覧（repo タグ・更新日 yyyy/MM/dd・ピン留め=右クリック「上位に固定」・defaults `artifactPins`・並びは純関数 `shelfListOrder` テスト済み・行クリック=ブラウザで開く・右クリック=開く/URLコピー/固定）。データは二源マージの永続インデックス — 源A=frames API（下記データ源）・源B=transcript 増分走査（`artifactPublishesFromSlice` の tool_use/tool_result ペアリング。**ペア持ち越し carry を scan state に永続化** — publish が走査境界を跨ぐとペアが組めず取りこぼすため）。マージ規則: title/updatedAt/softDeleted は API 勝ち、favicon/repoName は transcript 勝ち（favicon は API が穴埋め）。インデックスは `~/Library/Application Support/Shepherd/artifact-index.json`（追記型・softDeleted は減光表示で隠さない）。**検索は in-place 再描画**（`renderArtifactList` — rebuild すると入力中のフィールドが作り直されてフォーカスが落ちるため）で、rebuild ガードは「最終キー入力から2秒」の時間式（`lastArtifactTypeAt` — currentEditor ベースはフォーカスが残る限り盤面更新が無期限に止まる）。取得はセクションが開いている間だけ refresh() 便乗（API 15分・走査5分ガード・↻で強制・stale-while-error）。fixture モード（`SHEPHERD_PROJECTS_DIR` あり）では API を叩かない（実データの顧客名が docs スクショに写る事故防止）＋インデックスも scratch へ。（「更新セッション起動」導線も実装後にユーザー判断で撤去 — claude を立ち上げて URL を貼るだけの導線に価値がなかった）**リデプロイパルス**: 増分スキャンの delta で既知 URL への再 publish を検知（`artifactPulseAt`）→カードの Artifact バッジが数秒琥珀に点滅。
...
- **Claude OAuth API**（`Sources/ClaudeUsage.swift`・2026-07-16 拡張）: `GET api.anthropic.com/api/oauth/usage`（要 `anthropic-beta: oauth-2025-04-20` ヘッダ＋claude-code UA）でプラン使用量ダッシュボードの全データ — `limits[]`（session/weekly/モデル別の percent・resets_at・severity・**is_active**=いま支配的な窓〔実測2回とも消費%最大の窓に付いた。「進行中」ではない — session 4% 進行中でも false だった〕。緑点で可視化したが分かりにくくユーザー判断で**表示は不採用**・パースもしていない）と `spend`（クレジット: `used` は未契約でも $0.00 が返る・**`balance`/`cap` は未契約時 null**・有効時は `limit`=月間上限のマネーオブジェクト・`cap` は `{money,credits}` のネスト形で別物・**残額は API に無いので limit−used で計算**）。プランチップ（MAX 20X）は `GET /api/oauth/profile` の `organization.rate_limit_tier`（`planTierCache` で1日1回・失敗もキャッシュ）。パースは純関数 `parseUsage`（テスト済み）。取得は `maybeRefreshUsage`（60秒ガード・**stale-while-error**: 失敗 snapshot は lastUsage を上書きせず `usageError` に退避 — ゲージは旧値のまま減光＋「更新失敗」表示）。ダッシュボードの ↻（`forceDashboardRefresh`）は usage/planTier/accountCache の全ガードを無視して即再取得。**OAuth token の読み出しは `security` サブプロセスが第一経路**（下記地雷参照）。
- **Artifact frames API**（`Sources/ArtifactIndex.swift`）: `GET api.anthropic.com/api/frame/frames?limit=50`（同じ OAuth Bearer）で Artifact シェルフの源A。**`X-Frame-CP: go` / `X-Frame-Surface: code` / `X-Frame-Platform: cli` ヘッダが必須**（無いと 404。`anthropic-beta`・UA は不要 — 2026-07-17 probe 実測。ヘッダセットは claude CLI 内の実装から特定）。レスポンスは `{frames:[{slug,title,favicon,label,description,rel,updatedAt(ISO),created_at,view_count,audience,…}]}` — `softDeleted` は true のときだけ現れる・`rel=="mine"` に絞る・**窓は直近更新 50 件で「全部」は返らない**（ページネーション無し）。非公開 API なので `oauth/usage` と同じ規律: additive パース（`parseFrames` は形不明で nil 返し）・失敗はシェルフを源Bのみに縮退。取得はシェルフを開いた時だけ（15分ガード・refresh には乗せない — 開かない日はゼロコール）。
...
- **ポップオーバー表示中・ファイルドラッグ中は `rebuild` をスキップする**。rebuild するとアンカー/ドロップ先ビューが再生成されてポップオーバーが道連れで閉じる。`refresh()` のガード（`replyPopover == nil && repoPickerPopover == nil && dropPopover == nil && helpPopover == nil && lastArtifactTypeAt から2秒 && Date().timeIntervalSince(lastDragAt) > 1.0`）を壊さないこと（新しいポップオーバーを足したら必ずここに参加させる）。
...
- `SHEPHERD_DUMP=1` ＋上記 env/引数 — GUI を出さずにデモ盤面の行とツリーを stderr にダンプ（fixture の検証はまずこれ。GUI は watched dir を消すと更新が止まるので、fixture を作り直したらアプリも再起動する）。Artifact シェルフのインデックス（transcript 源のみ・決定的）も `ARTIFACTS N` として出る。
- `SHEPHERD_ARTIFACT_PROBE=1 Shepherd` — frames API の headless 実測（ヘッダ要件・レスポンス形を stderr に出して終了。token はプロセス内に閉じる）。
