...
- **ファミリーストリップ**（親カード最下部20px・2026-07-08）: `⌄/› 子 N 件・稼働中 M・待機 J`。J は **roster に残る idle teammate**（`SubagentRecord.onRoster && !working`・2026-07-14）で、カードは持たず、ホバーの覗き見（`showFamilyPeek` の `idle:` 引数・moon.zzz で薄色表示）と親カード右クリックメニューの per-teammate サブメニュー（メッセージ送信=inbox 注入で再稼働／片付け=team-lead の inbox に shutdown 依頼を注入。`AppDelegate+Teammates.swift`）に出る。折りたたみ中は子のステータスドット＋要注目の子の一言（blocked の `? 質問` 優先）も同居。クリックはストリップ全面でトグル — `RowView.hitTest` は NSButton 系しかクリックを通さないので**透明 `HoverButton` を全面に重ねる**構成。旧「右上⌄Nトグル」「折りたたみ時の別サマリカード（familySummaryView）」は廃止。
...
- **`~/.claude/teams/<team>/`**（`Sources/Teams.swift`・2026-07-14 実測）: agent teams の on-disk 状態。`config.json` = 名簿（**shutdown 承認でメンバーが削除される**ので「idle=名簿あり・再稼働可」と「終了=名簿なし」を区別できる。終了済みでも idle のままなら残り続ける）。`inboxes/<name>.json` = メンバーのメッセージキュー（`{"from","text","timestamp","msgV":1,"msg_id","type":"message","read":false}` の配列）。**外部プロセスがこの形式で追記すると idle teammate が resume して受信する**（party-game で end-to-end 実証）— HUD の「メッセージを送って再開」「片付け依頼（team-lead 宛・from "shepherd"）」の実体（`injectTeammateMessage`）。ただし無認証・無検証で、`text` 欠落エントリは受信側 resume を **`t.replace` でクラッシュさせ以後の配達を殺す**（probe 実測）ので、形式は実証済みの写しに固定し、パースできない既存 inbox には触らない。teammate の稼働判定にも名簿が効く: **名簿に居ない teammate は working にしない**（shutdown 承認ターンが idle_notification より後に jsonl を書くため、これが無いと終了後30分 working に張り付く）。
