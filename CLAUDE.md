...
- **消えたディレクトリを cwd に持つプロセスの子は起動直後に死ぬ**（2026-07-17 実測）。spawn 自体は成功するが、子が最初に自分の cwd を読んで即死する — `claude agents`（Bun）は `ENOENT: Bun could not find a file`、`git` は `fatal: Unable to read current working directory`。worktree 内から起動した Shepherd がその worktree の削除で全 subprocess を失い、盤面が rows=0 に空転した（`/tmp/shepherd-deck.log` で `rows=N → rows=0`・0.01s が兆候）。→ 対策は二重: 起動直後に `chdir("/")`（main.swift）＋ `runCommand` は cwd 未指定でも `/` を明示。
