...
- **`~/.claude/sessions/<pid>.json`**（`Sources/SessionsRegistry.swift`・2026-07-10）: **claude プロセス自身が書く** status レジストリ（`claude agents` が読んでいる実体）。`pid / sessionId / cwd / kind / name / status(busy|idle|waiting) / waitingFor("permission prompt" 等) / updatedAt`。サブプロセス不要のディレクトリ読み・リアルタイム更新・FSEvents 可。**フック未導入セッションの状態源**であり、blocked の理由（waitingFor）は `needs` のフォールバックにも使う。死んだ pid のファイルは skip（`kill(pid,0)`）。非公開ファイルなので additive に読む。
